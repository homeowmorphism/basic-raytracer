...
def ray_trace(origin, ray, scene):
    ...
    for prop in scene:
        prop_intersection = prop.find_intersection(origin, ray)
    ...
    if all(intersection is None for intersection in intersections):
        ...
        closest_prop = scene[index_closest_prop]
        ...
        if isinstance(closest_prop, Light):
            ...
            return lambertian*albedo.compwise_mult(ray_trace(new_origin + (epsilon*reflected_ray), reflected_ray, scene))

def shoot_rays(scene, screen_width, screen_height, screen_depth, iteration_by_pixel):
    ...
    for i in range(screen_width):
        for j in range(screen_height):
            ...
            ray = absolute_position
            sum_of_colors = vect(0,0,0)
            total_intensity = 0

            for k in range(iteration_by_pixel):
                color = ray_trace(absolute_position, ray, scene)
                sum_of_colors += color
